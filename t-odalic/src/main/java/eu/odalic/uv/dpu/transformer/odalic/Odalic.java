...
public class Odalic extends AbstractDpu<OdalicConfig_V1> {
  ...
  private void cancel(final String taskIdentifier, final Client client) throws DPUException {
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.execution.cancel.request");
    }

    if (!isSuccessful(response)) {
      throw ContextUtils.dpuException(this.ctx, "Odalic.error.execution.cancel.success");
    }
  }
  ...
  private void createAnnotatedTableOutput(final Client client, final String taskIdentifier)
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.output.creation.annotatedTable");
    }
  ...
  private void createExtendedCsvOutput(final Client client, final String taskIdentifier)
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.output.creation.extendedCsv");
    }
  ...
  private FilesDataUnit.Entry createOutputFile(final WritableFilesDataUnit unit,
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.output.creation", fileName);
    }
    return outputFile;
  }
  ...
  private void createTurtleOutput(final Client client, final String taskIdentifier)
      throws DPUException {
    final Entry entry = createOutputFile(this.turtleOutput, generateOutputFileIdentifier(
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.output.creation.turtle");
    }
  ...
  private void evaluateTerminalState(final State state, final String taskIdentifier,
    ...
    switch (state) {
      case SUCCESS:
        ContextUtils.sendInfo(this.ctx, "Odalic.result.success.short",
            "Odalic.result.success.full");
        break;
      case WARNING:
        final String[] warnings = getWarnings(taskIdentifier, client);
        ContextUtils.sendWarn(this.ctx, "Odalic.result.warnings.short",
            "Odalic.error.result.warnings.full", String.join(", ", warnings));
        break;
      case ERROR:
        final Message errorMessage = getErrorMessage(taskIdentifier, client);
        throw ContextUtils.dpuException(this.ctx, "Odalic.error.result.error",
            errorMessage.getText());
      default:
        throw ContextUtils.dpuException(this.ctx, "Odalic.error.state.invalid");
    }
  }

  private String getAuthorizationHeaderContent() {
    return String.format(AUTHORIZATION_HEADER_CONTENT_FORMAT, this.config.getToken());
  }

  private Message getErrorMessage(final String taskIdentifier, final Client client)
      throws DPUException {
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.message.request");
    }
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.message.processing");
    }
  ...
  private WebTarget getFilesTarget(final Client client) {
    return client.target(this.config.getHost()).path("files");
  }
  ...
  private File getInputFile() throws DPUException {
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, ex, "Odalic.error.input.get");
    }

    if (inputEntries.isEmpty()) {
      throw ContextUtils.dpuException(this.ctx, "Odalic.error.input.empty");
    }
    if (inputEntries.size() > 1) {
      throw ContextUtils.dpuException(this.ctx, "Odalic.error.input.multiple");
    }
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, "Odalic.error.input.conversion");
    }
  ...
  private Format getInputFileFormat() throws DPUException {
    ...
    try {
      format.setCharset(this.config.getCharset());
    } catch (final IllegalArgumentException | NullPointerException e) {
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.charset.invalid");
    }

    if (this.config.getDelimiter() == null) {
      throw ContextUtils.dpuException(this.ctx, "Odalic.error.delimiter.empty");
    }
    format.setDelimiter(this.config.getDelimiter());

    format.setCommentMarker(this.config.getCommentMarker());
    format.setEmptyLinesIgnored(this.config.isEmptyLinesIgnored());
    format.setEscapeCharacter(this.config.getEscapeCharacter());
    format.setQuoteCharacter(this.config.getQuoteCharacter());

    return format;
  }

  private State getState(final String taskIdentifier, final Client client) throws DPUException {
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.state.request");
    }

    if (!isSuccessful(response)) {
      throw ContextUtils.dpuException(this.ctx, "Odalic.error.state.success");
    }
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.state.processing");
    }
  ...
  private String getTaskConfiguration(final String inputFileIdentifier) throws DPUException {
    final String serialization = this.config.getTaskConfiguration();
    if (serialization == null) {
      throw ContextUtils.dpuException(this.ctx, "Odalic.error.taskConfiguration.empty");
    }
  ...
  private WebTarget getTasksTarget(final Client client) {
    return client.target(this.config.getHost()).path("tasks");
  }

  private String[] getWarnings(final String taskIdentifier, final Client client)
      throws DPUException {
    final WebTarget target = getTasksTarget(client).path(taskIdentifier).path("result");
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.result.request");
    }
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.result.processing");
    }
  ...
  private Model parseTaskConfiguration(final String serialization) throws DPUException {
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.taskConfiguration.parsing");
    }
  ...
  private State poll(final String taskIdentifier, final Client client) throws DPUException {
    ...
    while (state == State.RUNNING) {
      ...
      try {
        ...
        throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.interrupted");
      }
  ...
  private void sendInputFile(final String inputFileIdentifier, final File inputFile,
    ...
    try (final FormDataMultiPart multipartEntity = new FormDataMultiPart()) {
      ...
      try {
        ...
        throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.input.request");
      }

      if (!isSuccessful(response)) {
        throw ContextUtils.dpuException(this.ctx, "Odalic.error.input.upload.success");
      }
    } catch (final IOException e) {
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.input.multipart");
    }
  }
  ...
  private void sendInputFileFormat(final String inputFileIdentifier, final Format inputFileFormat,
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.format.request");
    }

    if (!isSuccessful(response)) {
      throw ContextUtils.dpuException(this.ctx, "Odalic.error.format.success");
    }
  }

  private void sendTaskConfigruation(final String taskIdentifier, final String taskConfiguration,
      final Client client) throws DPUException {
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.taskConfiguration.upload.request");
    }

    if (!isSuccessful(response)) {
      throw ContextUtils.dpuException(this.ctx, "Odalic.error.taskConfiguration.upload.success");
    }
  }

  private void startExecution(final String taskIdentifier, final Client client)
      throws DPUException {
    final WebTarget target = getTasksTarget(client).path(taskIdentifier).path("execution");
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.execution.request");
    }

    if (!isSuccessful(response)) {
      throw ContextUtils.dpuException(this.ctx, "Odalic.error.execution.success");
    }
  }
  ...
  private String toTurtle(final Model model) throws DPUException {
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.turtle.write");
    }
  ...
  private void writeAnnotatedTable(final String taskIdentifier, final Client client,
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.annotatedTable.request");
    }
    ...
    try (final InputStream entityStream = (InputStream) response.getEntity();
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.annotatedTable.processing");
    }
  }
  ...
  private void writeExtendedCsv(final String taskIdentifier, final Client client,
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.extendedCsv.request");
    }
    ...
    try (final InputStream entityStream = (InputStream) response.getEntity();
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.extendedCsv.processing");
    }
  }
  ...
  private void writeTurtle(final String taskIdentifier, final Client client, final File outputFile)
    ...
    try {
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.turtle.request");
    }
    ...
    try (final InputStream entityStream = (InputStream) response.getEntity();
      ...
      throw ContextUtils.dpuException(this.ctx, e, "Odalic.error.turtle.processing");
    }
  }
}
